...
void VectorScalarDiv(vector<float> &a, size_t x) {
  if (x == 0)
    return;
  for (auto &y : a) {
    y /= x;
...
auto FindCentroids(const std::vector<std::pair<vector<float>, RID>> &data, const std::vector<vector<float>> &centroids,
  ...
  std::random_device rd;  // Seed from a random device
  std::mt19937 gen(rd()); // Mersenne Twister engine for randomness
  auto five_per = static_cast<int>(data.size() * 0.05);
  
  std::uniform_int_distribution<> distr(1, data.size()); 
  for (int i = 0; i < five_per; i++) {
    int pos = distr(gen) % data.size();
    size_t index = FindCentroid(data[pos].first, res, dist_fn);
    VectorAdd(res[index],data[pos].first);
    count[index]++;
  }
...
vector<RID> IvfflatIndex::ScanVectorKey(const vector<float> &base_vector, size_t limit)  {
  ...
  std::sort(centroids.begin(), centroids.end(), [&](const auto &a, const auto &b) {
    auto dist_a = ComputeDistance(base_vector, centroids_[a], (int)distance_type_);
    auto dist_b = ComputeDistance(base_vector, centroids_[b], (int)distance_type_);
    return dist_a < dist_b;
  });
  std::vector<std::pair<vector<float>, RID>> selections;
  ...
  std::sort(selections.begin(), selections.end(), [&](const auto &a, const auto &b) {
    auto dist_a = ComputeDistance(base_vector, a.first, (int)distance_type_);
    auto dist_b = ComputeDistance(base_vector, b.first, (int)distance_type_);
    return dist_a < dist_b;
  });
  std::vector<RID> result;
  for (size_t i = 0; i < limit && i < selections.size(); i++) {
    result.push_back(selections[i].second);
    sql_debug("page num:%d, slot num:%d", result[i].page_num, result[i].slot_num );
  }
  return result;
}
