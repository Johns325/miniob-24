...
RC Db::create_view(const char *view_name, std::vector<std::string> &infos,bool has_schema,SelectSqlNode* select_node)
{
  RC rc = RC::SUCCESS;
  ...
  View *view=new View();
  rc=view->create(this,view_name,infos,has_schema,select_node);
  if (rc != RC::SUCCESS) {
