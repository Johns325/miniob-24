...
RC CreateViewExecutor::execute(SQLStageEvent *sql_event) 
{
  ...
  RC rc = session->get_current_db()->create_view(view_name, create_view_stmt->select_stmt(),create_view_stmt->infos(),create_view_stmt->has_schema(),create_view_stmt->select_node_);
